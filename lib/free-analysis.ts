...
const POSITIVE_WORDS = ["good", "great", "thanks", "perfect", "agreed", "done", "support", "help", "clear", "win"];
const NEGATIVE_WORDS = ["issue", "problem", "drop", "confusion", "friction", "delay", "risk", "wrong", "conflict", "fail"];
...
function pickTone(line: string): ToneLabel {
  ...
  if (/(thank|appreciate|help|support)/.test(lower)) return "Supportive";
  if (/(need|must|should|will|let's|lets|deadline)/.test(lower)) return "Assertive";
  if (/(analy|data|because|metric|suggest)/.test(lower)) return "Analytical";
  if (/(agree|together|we could|good idea)/.test(lower)) return "Collaborative";
  if (/(wrong|however|but|no)/.test(lower)) return "Defensive";
  return "Neutral";
}

function inferConversationType(text: string): ConversationType {
  const lower = text.toLowerCase();
  if (/(meeting|agenda|follow up|review)/.test(lower)) return "Meeting";
  if (/(launch|timeline|plan|prototype|next week)/.test(lower)) return "Planning";
  if (/(feedback|improve|suggestion)/.test(lower)) return "Feedback";
  if (/(support|ticket|customer)/.test(lower)) return "Support";
  if (/(price|deal|contract|budget)/.test(lower)) return "Negotiation";
  if (/(angry|conflict|argument|upset)/.test(lower)) return "Conflict";
  return "Casual";
}
...
function inferEmotionTags(text: string, positives: number, negatives: number): EmotionTag[] {
  ...
  if (/(agree|aligned|perfect|done)/.test(lower)) tags.push("Agreement");
  if (/(plan|next|action|review|prototype)/.test(lower)) tags.push("Productive");
  if (/(thanks|appreciate)/.test(lower)) tags.push("Appreciation");
  if (/(urgent|asap|today|immediately)/.test(lower)) tags.push("Urgency");
  if (/(confus|unclear)/.test(lower)) tags.push("Confusion");
  if (/(resolve|fixed|done)/.test(lower)) tags.push("Resolution");
...
function topTopics(text: string) {
  ...
  const stopWords = new Set(["this", "that", "with", "from", "have", "will", "your", "their", "there", "could", "would", "because", "need"]);
...
function actionItemsFromLines(lines: string[]) {
  return unique(
    lines.filter((line) => /(will|should|need to|let's|lets|follow up|review|send|share|draft|build|prepare)/i.test(line))
  ).slice(0, 4);
}
